        ...
        while (ros::ok())
        {
            ...
            while (!package_rcvd)
            {
                ...
                if (first_four_bytes == magicWord)
                {
                    ...
                    device_id = (uint8_t)read_header(msgBuf, header_msg);
            ...
            if (package_rcvd)
            {
                ...
                if (data_type == 0x2202)
                {
                    ...
                    for(int k = 0; k < lux_scan_msg.num_scan_pts; k++)
                    {
                        ...
                        scan_data.size = 2;
                        scan_point_data.horizontal_angle = (int16_t)read_value(msgBuf, scan_data);
                ...
                if (data_type == 0x2030)
                {
                    ...
                    sprintf(temp_value_string,"%x",(uint16_t)read_value(msgBuf, error_warn_data));
                ...
                if (data_type == 0x2204)
                {
                    ...
                    lux_fusion_scan_2204.scan_start_time = (uint64_t)read_value(msgBuf, scan_data);
                    scan_data.msgOffset = start_byte + 8;
                    scan_data.size = 4;
                    lux_fusion_scan_2204.scan_end_time_offset = (uint32_t)read_value(msgBuf, scan_data);
                    scan_data.msgOffset = start_byte  + 12;
                    lux_fusion_scan_2204.flags = (uint32_t)read_value(msgBuf, scan_data);
                    scan_data.msgOffset = start_byte  + 16;
                    scan_data.size = 2;
                    lux_fusion_scan_2204.scan_number = (uint16_t)read_value(msgBuf, scan_data);
                    scan_data.msgOffset = start_byte  + 18;
                    lux_fusion_scan_2204.num_scan_pts = (uint16_t)read_value(msgBuf, scan_data);
                    scan_data.msgOffset = start_byte  + 20;
                    scan_data.size = 1;
                    lux_fusion_scan_2204.num_scan_info = (uint8_t)read_value(msgBuf, scan_data);
                    ...
                    for(int k = 0; k < lux_fusion_scan_2204.num_scan_info; k++)
                    {
                        ...
                        scan_data.size = 2;
                        scan_info_2204.number = (uint16_t)read_value(msgBuf, scan_data);
                        scan_data.msgOffset = info_start_byte + 8;
                        scan_data.size = 4;
                        scan_info_2204.start_angle = (float)read_value(msgBuf, scan_data);
                        scan_data.msgOffset = info_start_byte + 12;
                        scan_info_2204.end_angle = (float)read_value(msgBuf, scan_data);
                        scan_data.msgOffset = info_start_byte + 16;
                        scan_info_2204.yaw_angle = (float)read_value(msgBuf, scan_data);
                        scan_data.msgOffset = info_start_byte + 20;
                        scan_info_2204.pitch_angle = (float)read_value(msgBuf, scan_data);
                        scan_data.msgOffset = info_start_byte + 24;
                        scan_info_2204.roll_angle = (float)read_value(msgBuf, scan_data);
                        scan_data.msgOffset = info_start_byte + 28;
                        scan_info_2204.mount_offset_x = (float)read_value(msgBuf, scan_data);
                        scan_data.msgOffset = info_start_byte + 32;
                        scan_info_2204.mount_offset_y = (float)read_value(msgBuf, scan_data);
                        scan_data.msgOffset = info_start_byte + 36;
                        scan_info_2204.mount_offset_z = (float)read_value(msgBuf, scan_data);
                    ...
                    {
                        //scan_data.msgOffset = scan_data.msgOffset + 10*k;
                        scan_point_2204.x_position = (float)read_value(msgBuf, scan_data);
                        scan_data.msgOffset = pt_start_byte + 4;
                        scan_point_2204.y_position = (float)read_value(msgBuf, scan_data);
                        scan_data.msgOffset = pt_start_byte + 8;
                        scan_point_2204.z_position = (float)read_value(msgBuf, scan_data);
                        scan_data.msgOffset = pt_start_byte + 12;
                        scan_point_2204.echo_width = (float)read_value(msgBuf, scan_data);
                        ...
                        scan_data.size = 2;
                        scan_point_2204.time_stamp = (uint32_t)read_value(msgBuf, scan_data);
                        scan_data.msgOffset = pt_start_byte + 24;
                        scan_data.size = 2;
                        scan_point_2204.flags = (uint16_t)read_value(msgBuf, scan_data);
                ...
                if (data_type == 0x2205)
                {
                    ...
                    lux_fusion_scan_2205.scan_start_time = (uint64_t)read_value(msgBuf, scan_data);
                    scan_data.msgOffset = start_byte + 8;
                    scan_data.size = 4;
                    lux_fusion_scan_2205.scan_end_time_offset = (uint32_t)read_value(msgBuf, scan_data);
                    scan_data.msgOffset = start_byte  + 12;
                    lux_fusion_scan_2205.flags = (uint32_t)read_value(msgBuf, scan_data);
                    scan_data.msgOffset = start_byte  + 16;
                    scan_data.size = 2;
                    lux_fusion_scan_2205.scan_number = (uint16_t)read_value(msgBuf, scan_data);
                    scan_data.msgOffset = start_byte  + 18;
                    lux_fusion_scan_2205.num_scan_pts = (uint16_t)read_value(msgBuf, scan_data);
                    scan_data.msgOffset = start_byte  + 20;
                    scan_data.size = 1;
                    lux_fusion_scan_2205.num_scan_info = (uint8_t)read_value(msgBuf, scan_data);
                    ...
                    for(int k = 0; k < lux_fusion_scan_2205.num_scan_info; k++)
                    {
                        ...
                        scan_data.msgOffset = info_start_byte + 2;
                        scan_data.size = 2;
                        scan_info_2205.number = (uint16_t)read_value(msgBuf, scan_data);
                        scan_data.msgOffset = info_start_byte + 8;
                        scan_data.size = 4;
                        scan_info_2205.start_angle = (float)read_value(msgBuf, scan_data);
                        scan_data.msgOffset = info_start_byte + 12;
                        scan_info_2205.end_angle = (float)read_value(msgBuf, scan_data);
                        scan_data.msgOffset = info_start_byte + 16;
                        scan_data.size = 8;
                        scan_info_2205.scan_start_time = (uint64_t)read_value(msgBuf, scan_data);
                        scan_data.msgOffset = info_start_byte + 24;
                        scan_info_2205.scan_end_time = (uint64_t)read_value(msgBuf, scan_data);
                        scan_data.msgOffset = info_start_byte + 32;
                        scan_info_2205.device_start_time = (uint64_t)read_value(msgBuf, scan_data);
                        scan_data.msgOffset = info_start_byte + 40;
                        scan_info_2205.device_end_time = (uint64_t)read_value(msgBuf, scan_data);
                        scan_data.msgOffset = info_start_byte + 48;
                        scan_data.size = 4;
                        scan_info_2205.scan_frequency = (float)read_value(msgBuf, scan_data);
                        scan_data.msgOffset = info_start_byte + 52;
                        scan_info_2205.beam_tilt = (float)read_value(msgBuf, scan_data);
                        scan_data.msgOffset = info_start_byte + 56;
                        scan_info_2205.scan_flags = (uint32_t)read_value(msgBuf, scan_data);
                        scan_data.msgOffset = info_start_byte + 60;
                        scan_info_2205.yaw_angle = (float)read_value(msgBuf, scan_data);
                        scan_data.msgOffset = info_start_byte + 64;
                        scan_info_2205.pitch_angle = (float)read_value(msgBuf, scan_data);
                        scan_data.msgOffset = info_start_byte + 68;
                        scan_info_2205.roll_angle = (float)read_value(msgBuf, scan_data);
                        scan_data.msgOffset = info_start_byte + 72;
                        scan_info_2205.offset_x = (float)read_value(msgBuf, scan_data);
                        scan_data.msgOffset = info_start_byte + 76;
                        scan_info_2205.offset_y = (float)read_value(msgBuf, scan_data);
                        scan_data.msgOffset = info_start_byte + 80;
                        scan_info_2205.offset_z = (float)read_value(msgBuf, scan_data);
                        scan_data.msgOffset = info_start_byte + 84;
                        scan_info_2205.resolution1.resolution_start_angle = (float)read_value(msgBuf, scan_data);
                        scan_data.msgOffset = info_start_byte + 88;
                        scan_info_2205.resolution1.resolution = (float)read_value(msgBuf, scan_data);

                        scan_data.msgOffset = info_start_byte + 92;
                        scan_info_2205.resolution2.resolution_start_angle = (float)read_value(msgBuf, scan_data);
                        scan_data.msgOffset = info_start_byte + 96;
                        scan_info_2205.resolution2.resolution = (float)read_value(msgBuf, scan_data);

                        scan_data.msgOffset = info_start_byte + 100;
                        scan_info_2205.resolution3.resolution_start_angle = (float)read_value(msgBuf, scan_data);
                        scan_data.msgOffset = info_start_byte + 104;
                        scan_info_2205.resolution3.resolution = (float)read_value(msgBuf, scan_data);

                        scan_data.msgOffset = info_start_byte + 108;
                        scan_info_2205.resolution4.resolution_start_angle = (float)read_value(msgBuf, scan_data);
                        scan_data.msgOffset = info_start_byte + 112;
                        scan_info_2205.resolution4.resolution = (float)read_value(msgBuf, scan_data);

                        scan_data.msgOffset = info_start_byte + 116;
                        scan_info_2205.resolution5.resolution_start_angle = (float)read_value(msgBuf, scan_data);
                        scan_data.msgOffset = info_start_byte + 120;
                        scan_info_2205.resolution5.resolution = (float)read_value(msgBuf, scan_data);

                        scan_data.msgOffset = info_start_byte + 124;
                        scan_info_2205.resolution6.resolution_start_angle = (float)read_value(msgBuf, scan_data);
                        scan_data.msgOffset = info_start_byte + 128;
                        scan_info_2205.resolution6.resolution = (float)read_value(msgBuf, scan_data);

                        scan_data.msgOffset = info_start_byte + 132;
                        scan_info_2205.resolution7.resolution_start_angle = (float)read_value(msgBuf, scan_data);
                        scan_data.msgOffset = info_start_byte + 136;
                        scan_info_2205.resolution7.resolution = (float)read_value(msgBuf, scan_data);

                        scan_data.msgOffset = info_start_byte + 140;
                        scan_info_2205.resolution8.resolution_start_angle = (float)read_value(msgBuf, scan_data);
                        scan_data.msgOffset = info_start_byte + 144;
                        scan_info_2205.resolution8.resolution = (float)read_value(msgBuf, scan_data);
                    ...
                    {
                        //scan_data.msgOffset = scan_data.msgOffset + 10*k;
                        scan_point_2205.x_position = (float)read_value(msgBuf, scan_data);
                        scan_data.msgOffset = pt_start_byte + 4;
                        scan_point_2205.y_position = (float)read_value(msgBuf, scan_data);
                        scan_data.msgOffset = pt_start_byte + 8;
                        scan_point_2205.z_position = (float)read_value(msgBuf, scan_data);
                        scan_data.msgOffset = pt_start_byte + 12;
                        scan_point_2205.echo_width = (float)read_value(msgBuf, scan_data);
                        scan_data.msgOffset = pt_start_byte + 16;
                        ...
                        scan_data.msgOffset = pt_start_byte + 20;
                        scan_data.size = 2;
                        scan_point_2205.time_stamp = (uint32_t)read_value(msgBuf, scan_data);
                        scan_data.msgOffset = pt_start_byte + 24;
                        scan_data.size = 2;
                        scan_point_2205.flags = (uint16_t)read_value(msgBuf, scan_data);
                ...
                if (data_type == 0x2225)
                {
                    ...
                    object_data.factor = 1;
                    object_data.valueOffset = 0;
                    lux_fusion_object_2225.mid_scan_timestamp = (uint64_t)read_value(msgBuf, object_data);
                    object_data.msgOffset = start_byte + 8;
                    object_data.size = 2;
                    lux_fusion_object_2225.num_of_objects = (uint16_t)read_value(msgBuf, object_data);
                    ...
                    for(int k = 0; k < lux_fusion_object_2225.num_of_objects; k++)
                    {
                        ...
                        object_data.msgOffset = object_start_byte + 4;
                        object_data.size = 4;
                        scan_object.object_age = (uint32_t)read_value(msgBuf, object_data);
                        object_data.msgOffset = object_start_byte + 8;
                        object_data.size = 8;
                        scan_object.time_stamp = (uint64_t)read_value(msgBuf, object_data);
                        object_data.msgOffset = object_start_byte + 16;
                        object_data.size = 2;
                        scan_object.object_hidden_age = (uint16_t)read_value(msgBuf, object_data);
                        object_data.msgOffset = object_start_byte + 18;
                        ...
                        object_data.msgOffset = object_start_byte + 20;
                        object_data.size = 4;
                        scan_object.calssifciation_age = (uint32_t)read_value(msgBuf, object_data);
                        object_data.msgOffset = object_start_byte + 24;
                        scan_object.bounding_box_center.x = (float)read_value(msgBuf, object_data);
                        object_data.msgOffset = object_start_byte + 28;
                        scan_object.bounding_box_center.y = (float)read_value(msgBuf, object_data);
                        object_data.msgOffset = object_start_byte + 32;
                        scan_object.bounding_box_size.x = (float)read_value(msgBuf, object_data);
                        object_data.msgOffset = object_start_byte + 36;
                        scan_object.bounding_box_size.y = (float)read_value(msgBuf, object_data);
                        object_data.msgOffset = object_start_byte + 40;
                        scan_object.object_box_center.x = (float)read_value(msgBuf, object_data);
                        object_data.msgOffset = object_start_byte + 44;
                        scan_object.object_box_center.y = (float)read_value(msgBuf, object_data);
                        object_data.msgOffset = object_start_byte + 48;
                        scan_object.object_box_center_sigma.x = (float)read_value(msgBuf, object_data);
                        object_data.msgOffset = object_start_byte + 52;
                        scan_object.object_box_center_sigma.y = (float)read_value(msgBuf, object_data);
                        object_data.msgOffset = object_start_byte + 56;
                        scan_object.object_box_size.x = (float)read_value(msgBuf, object_data);
                        object_data.msgOffset = object_start_byte + 60;
                        scan_object.object_box_size.y = (float)read_value(msgBuf, object_data);
                        object_data.msgOffset = object_start_byte + 72;
                        scan_object.yaw_angle = (float)read_value(msgBuf, object_data);
                        object_data.msgOffset = object_start_byte + 80;
                        scan_object.relative_velocity.x = (float)read_value(msgBuf, object_data);
                        object_data.msgOffset = object_start_byte + 84;
                        scan_object.relative_velocity.y = (float)read_value(msgBuf, object_data);
                        object_data.msgOffset = object_start_byte + 88;
                        scan_object.relative_velocity_sigma.x = (float)read_value(msgBuf, object_data);
                        object_data.msgOffset = object_start_byte + 92;
                        scan_object.relative_velocity_sigma.y = (float)read_value(msgBuf, object_data);
                        object_data.msgOffset = object_start_byte + 96;
                        scan_object.absolute_velocity.x = (float)read_value(msgBuf, object_data);
                        object_data.msgOffset = object_start_byte + 100;
                        scan_object.absolute_velocity.y = (float)read_value(msgBuf, object_data);
                        object_data.msgOffset = object_start_byte + 104;
                        scan_object.absolute_velocity_sigma.x = (float)read_value(msgBuf, object_data);
                        object_data.msgOffset = object_start_byte + 108;
                        scan_object.absolute_velocity_sigma.y = (float)read_value(msgBuf, object_data);
                        ...
                        for(int j =0; j< scan_object.number_of_contour_points; j++)
                        {
                            object_point.x = (float)read_value(msgBuf, object_data);
                            object_data.msgOffset = pt_start_byte + 4;
                            object_point.y = (float)read_value(msgBuf, object_data);
                            scan_object.list_of_contour_points.push_back(object_point);
                            object_data.msgOffset = pt_start_byte + 4;
                        }
                ...
                if (data_type == 0x2280)
                {
                    ...
                    object_data.factor = 1;
                    object_data.valueOffset = 0;
                    lux_fusion_object_2280.mid_scan_timestamp = (uint64_t)read_value(msgBuf, object_data);
                    object_data.msgOffset = start_byte + 8;
                    object_data.size = 2;
                    lux_fusion_object_2280.num_of_objects = (uint16_t)read_value(msgBuf, object_data);
                    ...
                    for(int k = 0; k < lux_fusion_object_2280.num_of_objects; k++)
                    {
                        scan_object.ID = (uint16_t)read_value(msgBuf, object_data);
                        object_data.msgOffset = object_start_byte + 2;
                        scan_object.flags = (uint16_t)read_value(msgBuf, object_data);
                        object_data.msgOffset = object_start_byte + 4;
                        object_data.size = 4;
                        scan_object.object_age = (uint32_t)read_value(msgBuf, object_data);
                        object_data.msgOffset = object_start_byte + 8;
                        ...
                        object_data.msgOffset = object_start_byte + 16;
                        object_data.size = 2;
                        scan_object.object_prediction_age = (uint16_t)read_value(msgBuf, object_data);
                        object_data.msgOffset = object_start_byte + 18;
                        scan_object.classification = (uint8_t)msgBuf[object_data.msgOffset];
                        object_data.msgOffset = object_start_byte + 19;
                        ...
                        scan_object.object_box_size.y = (float)read_value(msgBuf, object_data);
                        object_data.msgOffset = object_start_byte + 72;
                        scan_object.object_course_angle = (float)read_value(msgBuf, object_data);
                        object_data.msgOffset = object_start_byte + 76;
                        scan_object.object_course_angle_sigma = (float)read_value(msgBuf, object_data);
                        object_data.msgOffset = object_start_byte + 80;
                        scan_object.relative_velocity.x = (float)read_value(msgBuf, object_data);
                        object_data.msgOffset = object_start_byte + 84;
                        scan_object.relative_velocity.y = (float)read_value(msgBuf, object_data);
                        ...
                        object_data.size = 2;
                        scan_object.reference_point_location = (uint16_t)read_value(msgBuf, object_data);
                        object_data.msgOffset = object_start_byte + 134;
                        object_data.size = 4;
                        scan_object.reference_point_coordinate.x = (float)read_value(msgBuf, object_data);
                        object_data.msgOffset = object_start_byte + 138;
                        scan_object.reference_point_coordinate.y = (float)read_value(msgBuf, object_data);
                        object_data.msgOffset = object_start_byte + 142;
                        scan_object.reference_point_coordinate_sigma.x = (float)read_value(msgBuf, object_data);
                        object_data.msgOffset = object_start_byte + 146;
                        scan_object.reference_point_coordinate_sigma.y = (float)read_value(msgBuf, object_data);
                        object_data.msgOffset = object_start_byte + 162;
                        object_data.size = 2;
                        scan_object.object_priority = (uint16_t)read_value(msgBuf, object_data);
                ...
                if (data_type == 0x2403)
                {
                    ...
                    lux_fusion_image_2403.image_format = (uint16_t)read_value(msgBuf, image_data);
                    image_data.msgOffset = start_byte + 2;
                    image_data.size = 4;
                    lux_fusion_image_2403.time_stamp = (uint32_t)read_value(msgBuf, image_data);
                    image_data.msgOffset = start_byte  + 6;
                    image_data.size = 8;
                    lux_fusion_image_2403.time_stamp_ntp = (uint64_t)read_value(msgBuf, image_data);
                    ...
                    image_data.size = 4;
                    lux_fusion_image_2403.mouting_position.yaw_angle = (float)read_value(msgBuf, image_data);
                    image_data.msgOffset = start_byte  + 19;
                    lux_fusion_image_2403.mouting_position.pitch_angle = (float)read_value(msgBuf, image_data);
                    image_data.msgOffset = start_byte  + 23;
                    lux_fusion_image_2403.mouting_position.roll_angle = (float)read_value(msgBuf, image_data);
                    image_data.msgOffset = start_byte  + 27;
                    lux_fusion_image_2403.mouting_position.offset_x = (float)read_value(msgBuf, image_data);
                    image_data.msgOffset = start_byte  + 31;
                    lux_fusion_image_2403.mouting_position.offset_y = (float)read_value(msgBuf, image_data);
                    image_data.msgOffset = start_byte  + 35;
                    lux_fusion_image_2403.mouting_position.offset_z = (float)read_value(msgBuf, image_data);
                    image_data.msgOffset = start_byte  + 39;
                    image_data.size = 8;
                    lux_fusion_image_2403.horizontal_opening_angle = read_value(msgBuf, image_data);
                    image_data.msgOffset = start_byte  + 47;
                    lux_fusion_image_2403.vertical_opening_angle = read_value(msgBuf, image_data);
                    image_data.msgOffset = start_byte  + 55;
                    image_data.size = 2;
                    lux_fusion_image_2403.image_width = (uint16_t)read_value(msgBuf, image_data);
                    image_data.msgOffset = start_byte  + 57;
                    lux_fusion_image_2403.image_height = (uint16_t)read_value(msgBuf, image_data);
                    image_data.msgOffset = start_byte  + 59;
                    image_data.size = 4;
                    lux_fusion_image_2403.compress_size = (uint32_t)read_value(msgBuf, image_data);
                ...
                if (data_type == 0x2806)
                {
                    ...
                    vehicle_state_data.factor = 1;
                    vehicle_state_data.valueOffset = 0;
                    lux_vehicle_state_msg.time_stamp = (uint64_t)read_value(msgBuf, vehicle_state_data);
                    vehicle_state_data.msgOffset = start_byte + 12;
                    vehicle_state_data.size = 4;
                    lux_vehicle_state_msg.distance_x = (int32_t)read_value(msgBuf, vehicle_state_data);
                    vehicle_state_data.msgOffset = start_byte + 16;
                    lux_vehicle_state_msg.distance_y = (int32_t)read_value(msgBuf, vehicle_state_data);
                    vehicle_state_data.msgOffset = start_byte + 20;
                    lux_vehicle_state_msg.course_angle = (float)read_value(msgBuf, vehicle_state_data);
                    vehicle_state_data.msgOffset = start_byte + 24;
                    lux_vehicle_state_msg.longitudinal_velocity = (float)read_value(msgBuf, vehicle_state_data);
                    vehicle_state_data.msgOffset = start_byte + 28;
                    lux_vehicle_state_msg.yaw_rate = (float)read_value(msgBuf, vehicle_state_data);
                    vehicle_state_data.msgOffset = start_byte + 32;
                    lux_vehicle_state_msg.steering_wheel_angle = (float)read_value(msgBuf, vehicle_state_data);
                    vehicle_state_data.msgOffset = start_byte + 40;
                    lux_vehicle_state_msg.front_wheel_angle = (float)read_value(msgBuf, vehicle_state_data);
                    vehicle_state_data.msgOffset = start_byte + 46;
                    lux_vehicle_state_msg.vehicle_width = (float)read_value(msgBuf, vehicle_state_data);
                    vehicle_state_data.msgOffset = start_byte + 54;
                    lux_vehicle_state_msg.front_axle_to_front_axle = (float)read_value(msgBuf, vehicle_state_data);
                    vehicle_state_data.msgOffset = start_byte + 58;
                    lux_vehicle_state_msg.rear_axle_to_front_axle = (float)read_value(msgBuf, vehicle_state_data);
                    vehicle_state_data.msgOffset = start_byte + 62;
                    lux_vehicle_state_msg.rear_axle_to_vehicle_rear = (float)read_value(msgBuf, vehicle_state_data);
                    vehicle_state_data.msgOffset = start_byte + 70;
                    lux_vehicle_state_msg.steer_ratio_poly0 = (float)read_value(msgBuf, vehicle_state_data);
                    vehicle_state_data.msgOffset = start_byte + 74;
                    lux_vehicle_state_msg.steer_ratio_poly1 = (float)read_value(msgBuf, vehicle_state_data);
                    vehicle_state_data.msgOffset = start_byte + 78;
                    lux_vehicle_state_msg.steer_ratio_poly2 = (float)read_value(msgBuf, vehicle_state_data);
                    vehicle_state_data.msgOffset = start_byte + 82;
                    lux_vehicle_state_msg.steer_ratio_poly3 = (float)read_value(msgBuf, vehicle_state_data);
                ...
                if (data_type == 0x2807)
                {
                    ...
                    lux_vehicle_state_msg.lobitudinal_accelration = (float)read_value(msgBuf, vehicle_state_data);
